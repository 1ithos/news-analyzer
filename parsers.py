...
class BaseParser(ABC):
    def __init__(self, site_config: dict, global_config: dict):
        ...
        :param site_config: 当前网站的特定配置 (来自SITES_CONFIG列表的一项)。
        :param global_config: 整个config.yaml加载后的全局配置字典。
        """
        self.config = site_config
        self.headers = {'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36'}

        # 从全局配置中读取代理信息
        proxy_config = global_config.get('proxy')
        if proxy_config and isinstance(proxy_config, dict):
            self.proxies = proxy_config
            print(f"解析器已为【{self.config['name']}】配置代理: {self.proxies}")
        else:
            self.proxies = None  # 如果未配置代理，则为None
...
class StandardRSSParser(BaseParser):
    ...
    def parse(self) -> pd.DataFrame:
        ...
        try:
            ...
            feed = feedparser.parse(self.config['url'], request_headers=self.headers, handlers=handlers)
            ...
            for entry in feed.entries:
                try:
                    ...
                    logging.error(f"StandardRSSParser处理条目时出错: {e}")
                    continue
            return pd.DataFrame(articles)
        except Exception as e:
            logging.error(f"StandardRSSParser在代理模式下解析【{self.config['name']}】失败: {str(e)}")
            return pd.DataFrame()
        ...
        try:
            ...
            for entry in feed.entries:
                try:
                    ...
                    logging.error(f"GeekParkRSSParser处理条目时出错: {e}, URL: {entry.get('link', 'N/A')}")
                    continue

            return pd.DataFrame(processed_articles)

        except Exception as e:
            logging.error(f"GeekParkRSSParser解析【{self.config['name']}】失败: {str(e)}")
            return pd.DataFrame()
        ...
        try:
            ...
            for entry in feed.entries:
                try:
                    ...
                    logging.error(f"  > [错误] 处理条目时发生意外: {e}")
                    continue

            return pd.DataFrame(processed_articles)

        except Exception as e:
            logging.error(f"ReutersParser解析【{self.config['name']}】失败: {str(e)}")
            return pd.DataFrame()