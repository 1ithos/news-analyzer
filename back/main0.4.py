...
def main():
    ...
    logging.info(f"日期过滤完成，共 {len(today_combined_df)} 篇今天发布的新闻。")
    ...
    logging.info(f"去重完成，移除了 {initial_count - final_count} 篇。剩余 {final_count} 篇。")
    ...
    if frequency_bonus.sum() > 0: logging.info("已为高频标题增加重要性分数。")

    # 4. 应用独立的强制保留规则
    forced_df, candidate_df = apply_force_keep_rules(classified_df, FORCE_KEEP_RULES)
    logging.info(f"筛选出 {len(forced_df)} 篇强制保留文章。")
    ...
    logging.info(f"根据配额从候选文章中选出 {len(quota_selection_df)} 篇。")
